...
class PackingBox(Base):
    ...
    fecha_apertura = Column(DateTime, nullable=False, default=lambda: datetime.now(timezone.utc))
...
class ProductLocation(Base):
    ...
    # Última vez que se actualizó el stock de esta ubicación
    ultima_actualizacion_stock = Column(DateTime, nullable=True)
    
    # Observaciones o notas sobre esta ubicación
    # Ejemplo: "Zona de alta rotación", "Revisar vencimiento", etc.
    ...
    def codigo_ubicacion(self):
        """Genera código legible de la ubicación completa"""
        parts = []
        if self.pasillo:
            parts.append(self.pasillo)
        if self.lado:
            parts.append(self.lado[:3].upper())  # IZQ, DER
        if self.ubicacion:
            parts.append(self.ubicacion)
        if self.altura:
            parts.append(f"H{self.altura}")
...
class ReplenishmentRequest(Base):
    """
    Stock replenishment requests between locations.
    ...
    - REJECTED: Rejected (e.g., discontinued product)
    """
    __tablename__ = "replenishment_requests"
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    
    # Origin: replenishment zone (can be NULL if no stock)
    location_origen_id = Column(Integer, ForeignKey("product_locations.id", ondelete="NO ACTION"), nullable=True, index=True)
    # Destination: picking zone (always present)
    location_destino_id = Column(Integer, ForeignKey("product_locations.id", ondelete="NO ACTION"), nullable=False, index=True)
    ...
    # Relationships
    location_origin = relationship("ProductLocation", foreign_keys=[location_origen_id])
    location_destination = relationship("ProductLocation", foreign_keys=[location_destino_id])
    product = relationship("ProductReference")
    requester = relationship("Operator", foreign_keys=[requester_id])
    executor = relationship("Operator", foreign_keys=[executor_id])
    order = relationship("Order")
    
    __table_args__ = (
        Index('idx_status_priority_date', 'status', 'priority', 'requested_at'),
        Index('idx_product_destination', 'product_id', 'location_destino_id'),
        Index('idx_requester_status', 'requester_id', 'status'),
    )
    
    def __repr__(self):
        return f"<ReplenishmentRequest {self.id} {self.status} product_id={self.product_id}>"
